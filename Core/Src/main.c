...
int SHT40_Read(float *temperature, float *humidity)
{
  uint8_t cmd = 0xFD; // low repeatability
  ...
  *temperature = -45 + 175 * ((float)t_raw / 65535);
  *humidity = 100 * ((float)h_raw / 65535);
  return HAL_OK;
}
...
int main(void)
{
  ...
  MX_RTC_Init();
  /* USER CODE BEGIN 2 */
  printf("\r\n=== STM32L412 + PB0 Interrupt + SHT40 ===\r\n");
  printf("Press button on PB0 (D3) to wake/read sensor.\r\n");
  /* USER CODE END 2 */
  ...
  while (1)
  {
    printf("Going to SLEEP mode... Press PB0 to wake!\r\n");
    HAL_Delay(300);
